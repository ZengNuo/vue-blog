<template>
  <div id='article-edit'>
    <div class='wrap'>
      ...
      <div class="save">
        <input @click.prevent="updateArticle" class="button_submit" type="button" value="保存" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    updateArticle () {
      axios.put('/api/article?id=' + this.$route.query.id, this.article, {
        headers: {
        ...
        if (json.status === 1) {
          this.$router.push({
            path: '/article/detail?id=' + this.$route.query.id
          })
        }
      })
    }
